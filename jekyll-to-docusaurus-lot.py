...
if __name__ == "__main__":
    ...
    def find_markdown_files(folder: Path) -> List[Path]:
        """Retourne la liste des *.md* du dossier (récursif)."""
...
def convert_directory(
    ...
    """Convertit tous les fichiers Markdown d'un dossier."""
    conv = JekyllToDocusaurusConverter()
    ...
    print(f"📂 {len(md_files)} fichier(s) à convertir…")
    ...
    for md in md_files:
        ...
        if out_path.exists() and not overwrite:
            print(f"⚠️  {out_path} existe déjà – ignoré")
            continue

        success = conv.convert_file(md, out_path)
        status = "✅" if success else "❌"
        print(f"{status} {rel_path}")

    print("\n🗒️  Journal de conversion :")
    for line in conv.conversion_log:
        print("  •", line)
...
if __name__ == "__main__":
    # Chemins par défaut
    default_in = Path("convert-md/input")
    default_out = Path("convert-md/output")

    # Paramètres CLI :
    #   python batch_convert.py [input_dir] [output_dir] [--overwrite]
    in_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else default_in
    out_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else default_out
    ow_flag = "--overwrite" in sys.argv

    t0 = datetime.now()
    print("🚀 Batch conversion Jekyll → Docusaurus")
    print(f"📖 Dossier source : {in_dir.resolve()}")
    print(f"📝 Dossier cible : {out_dir.resolve()}\n")

    convert_directory(in_dir, out_dir, overwrite=ow_flag)

    dt = (datetime.now() - t0).total_seconds()
    print(f"\n⏱️  Terminé en {dt:.1f} s")